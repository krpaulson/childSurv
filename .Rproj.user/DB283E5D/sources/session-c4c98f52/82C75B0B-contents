...
#'
#' @examples
log_logistic <- function(data_vr, time_model, start_year = 1950, end_year = 2030) {
  ...
  # } else if (time_model == "rw2") {
    ...
    data_list <- list(model = "ll_vr_rw",
                      ...
                      pop = data_vr$pop)
    param_list <- list(intercept_log_shape = -1,
                       intercept_log_scale = 15,
                       log_tau_delta_log_shape = 0,
                       log_tau_delta_log_scale = 0,
  ...
  opt <- nlminb(obj$par, obj$fn, obj$gr,
                lower     = c(-60,0, rep(-60,4)),
                upper     = c(0,600, rep(100,4)))
